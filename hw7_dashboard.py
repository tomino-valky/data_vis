        ...
        fig_par = go.Figure(data=
            go.Parcoords(
                line=dict(color=df_ml['outcome_num'],
                          ...
                          showscale=True, 
                          colorbar=dict(title='Outcome (0=Lived, 2=Died)', tickvals=[0,1,2], ticktext=['Lived', 'Euthanized', 'Died'])
                         ),
                dimensions=list([
                    dict(range=[df_ml['pulse'].min(), df_ml['pulse'].max()], label='Pulse', values=df_ml['pulse']),
                    dict(range=[df_ml['rectal_temp'].min(), df_ml['rectal_temp'].max()], label='Rectal Temp', values=df_ml['rectal_temp']),
                    dict(range=[df_ml['respiratory_rate'].min(), df_ml['respiratory_rate'].max()], label='Respiratory Rate', values=df_ml['respiratory_rate']),
                    dict(range=[df_ml['total_protein'].min(), df_ml['total_protein'].max()], label='Total Protein', values=df_ml['total_protein']),
                    dict(range=[df_ml['packed_cell_volume'].min(), df_ml['packed_cell_volume'].max()], label='PCV', values=df_ml['packed_cell_volume'])
                ])
            )
        )
